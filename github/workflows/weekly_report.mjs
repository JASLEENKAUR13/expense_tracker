...
for (const user of users) {
  try {
    ...
    console.log(`Found ${expenses.length} expenses for ${user.user_name}`);

    if (expenses.length === 0) {
      console.log(`Skipping ${user.user_name} - no expenses`);
      continue;
    }
    ...
    if (user.onesignal_player_id) {
      await fetch('https://onesignal.com/api/v1/notifications', {
        ...
        body: JSON.stringify({
          ...
          contents: { en: `Hey ${user.user_name?.split(' ')[0] || 'there'}! Your weekly expense report is ready. Tap to view.` }
        })
      });
